...
aadt_lookup = {}
for _, row in aadt_2021.iterrows():
    aadt_lookup[(row["LHRS"], row["Offset"])] = row["AADT_clean"]
...
for hwy in target_highways:
    ...
    for route_num, seg_type, l_muni, r_muni, seg_len, aadt, desc, street in rows:
        ...
        if seg_type == "Ramp" and street and street_names_map[hwy] in street.upper():
            ramp_count += 1
...
fields = []
for f in arcpy.ListFields("highways_classified_v2"):
    fields.append(f.name)

matching_fields = []
for f in fields:
    if "AADT" in f.upper() or "LENGTH" in f.upper():
